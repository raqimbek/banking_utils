...
public class PaymentSystemHandler implements HttpHandler {
  ...
  public void handle(HttpExchange exchange) throws IOException {
    if ("POST".equals(exchange.getRequestMethod())) {
      ...
      var cardValidationInfo = cardValidator.validate(Arrays.asList(cardNumber.split("")));
      ...
      if (cardValidationInfo.isValid()) {
        ...
        if (paymentSystemOptional.isPresent()) {
          response.put("is-payment-system-determination-successfull", true);
          response.put("payment-system", paymentSystemOptional.get().toString());
          httpResponder.respondJson(exchange, response, 200);
        } else {
          response.put("is-payment-system-determination-successfull", false);
          response.put("errors", "Something went wrong... Payment system could not be determined.");
